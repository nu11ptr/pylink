...
fn run_python(code: &str) {
    ...
    // SAFETY: pylink initialized CPython, and this scope acquires and releases
...
fn incomplete_python_home(name: &str) -> std::path::PathBuf {
    let home = std::env::temp_dir().join(format!("pylink-{name}-{}", std::process::id()));
    let minor_version = pylink::PYTHON_VERSION
        .split('.')
        .take(2)
        .collect::<Vec<_>>()
        .join(".");
...
fn initializes_isolated_python_and_releases_the_gil_for_other_threads() {
    let home = pylink::runtime_home().unwrap();
    assert!(matches!(
        pylink::initialize_from("bad\0path"),
        Err(pylink::Error::InvalidPath(_))
    ));
    #[cfg(unix)]
    {
        use std::os::unix::ffi::OsStrExt;
        assert!(matches!(
            pylink::initialize_from(std::ffi::OsStr::from_bytes(b"bad\xffpath")),
            Err(pylink::Error::NonUnicodePath(_))
        ));
    }
    #[cfg(windows)]
    {
        ...
        assert!(matches!(
            pylink::initialize_from(std::ffi::OsString::from_wide(&[0xd800])),
            Err(pylink::Error::NonUnicodePath(_))
        ));
    }
    let missing = home.join("this-python-home-does-not-exist");
    assert!(matches!(
        pylink::initialize_from(missing),
        Err(pylink::Error::InvalidHome { .. })
    ));
    pylink::initialize().unwrap();
    pylink::initialize().unwrap();
    pylink::initialize_from(&home).unwrap();
    assert_eq!(pylink::runtime_home().unwrap(), home);
    let other_home = incomplete_python_home("other-python-home");
    assert!(matches!(
        pylink::initialize_from(&other_home),
        Err(pylink::Error::AlreadyInitialized { .. })
    ));
...
fn initialization_ignores_python_environment_settings() {
    const CHILD_MARKER: &str = "PYLINK_TEST_ISOLATED_ENVIRONMENT";
    const IGNORED_PATH: &str = "pylink-pythonpath-must-be-ignored";
    ...
    let output = std::process::Command::new(std::env::current_exe().unwrap())
        ...
        .env(CHILD_MARKER, "1")
        .env("PYTHONHOME", "pylink-invalid-python-home")
...
fn initialization_failure_returns_an_error_without_exiting_the_process() {
    const CHILD_MARKER: &str = "PYLINK_TEST_FAILED_INITIALIZATION";
    if std::env::var_os(CHILD_MARKER).is_some() {
        ...
        let first = pylink::initialize_from(&home).unwrap_err();
        assert!(matches!(first, pylink::Error::Initialization(_)));
        assert_eq!(pylink::initialize_from(&home).unwrap_err(), first);
